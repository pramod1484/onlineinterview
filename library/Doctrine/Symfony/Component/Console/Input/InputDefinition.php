    ...
    public function asText()
    {
        ...
        if ($this->getOptions()) {
            ...
            foreach ($this->getOptions() as $option) {
                ...
                $text[] = sprintf(" <info>%s</info> %-${optionMax}s%s%s%s", '--' . $option->getName(), $option->getShortcut() ? sprintf('(-%s) ', $option->getShortcut()) : '', $description, $default, $multiple
                );
            }
    ...
    public function asXml($asDom = false)
    {
        ...
        foreach ($this->getArguments() as $argument) {
            ...
            $defaults = is_array($argument->getDefault()) ? $argument->getDefault() : (is_bool($argument->getDefault()) ? array(
                        var_export($argument->getDefault(), true)) : ($argument->getDefault() ? array(
        ...
        foreach ($this->getOptions() as $option) {
            ...
            if ($option->acceptValue()) {
                ...
                $defaults = is_array($option->getDefault()) ? $option->getDefault() : (is_bool($option->getDefault()) ? array(
                            var_export($option->getDefault(), true)) : ($option->getDefault() ? array(
