...
class LimitSubqueryOutputWalker extends SqlWalker
{
    ...
    public function walkSelectStatement(SelectStatement $AST)
    {
        ...
        foreach ($rootIdentifier as $property) {
            if (isset($rootClass->fieldMappings[$property])) {
                foreach (array_keys($this->rsm->fieldMappings, $property) as
                            $alias) {
            ...
            if (isset($rootClass->associationMappings[$property])) {
                ...
                foreach (array_keys($this->rsm->metaMappings, $joinColumn) as
                            $alias) {
                    if ($this->rsm->columnOwnerMap[$alias] == $rootAlias) {
                        $sqlIdentifier[$property] = $alias;
                    }
        ...
        if (count($rootIdentifier) != count($sqlIdentifier)) {
            throw new \RuntimeException(sprintf(
                    'Not all identifier properties can be found in the ResultSetMapping: %s', implode(', ', array_diff($rootIdentifier, array_keys($sqlIdentifier)))
            ));
        }
    ...
    public function getPostgresqlSql(SelectStatement $AST, array $sqlIdentifier, $innerSql, &$sql)
    {
        ...
        if (count($orderBy)) {
            $sql = sprintf(
                    'SELECT DISTINCT %s FROM (%s) dctrn_result ORDER BY %s', implode(', ', array_merge($sqlIdentifier, $sqlOrderColumns)), $innerSql, implode(', ', $orderBy)
            );
        }
    }
