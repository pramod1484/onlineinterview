...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => $include_directory . '../data/upload/', //dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/',
            'upload_url' => BASEPATH . 'data/upload/', //$this->get_full_url().'/files/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    function addFileDB($name, $type, $size) {
        global $db;
        $sql = "INSERT INTO `attachment`(`id`, `name`, `type`, `size`, `created_at`) 
                VALUES (NULL, '$name', '$type', '$size', now())";
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $insertId = $this->addFileDB($name, $type, $size);
        $file = new \stdClass();
        $file->name = md5($insertId); /*$this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range);*/
        $file->size = $this->fix_integer_overflow(intval($size));
        $file->type = $type;
        $file->savedName = $this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range);
        $file->insertId = $insertId;
        //echo "Vitthal: ";        var_dump($file); exit;
