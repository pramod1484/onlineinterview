...
if (!empty($_POST)) {
    ...
    if (!$response['valid']) {
        ...
        redirect('administration/modules/status/edit.php?id=' . $_POST['status_id']);
    } else {
        $fields = array();
        ...
        updateStatusDetails($fields, $where);

        redirect('administration/modules/status/view.php?id=' . $_POST['status_id']);
    }
}