...
if (!empty($_POST)) {
    ...
    if (!$response['valid']) { 
        ...
        redirect('administration/modules/teams/edit.php?id='.$_POST['team']);
    } else {
        $fields = array();
        $fields['name'] = escapeString($_POST['name']);
        
        $where = array('id' => (int) $_POST['team']);
        // updating team details now
        updateTeamDetails($fields, $where);
        
        redirect('administration/modules/teams/view.php?id='.$_POST['team']);
    }    
}